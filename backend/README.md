...
У проєкті реалізовано дві сутності, дані яких зберігаються в оперативній пам'яті:
...
The project implements two entities whose data is stored in RAM:
...
3. Examples of using requests (curl):
IMPORTANT! When working with VS Code on Windows, you MUST add .exe to the command (curl.exe), or execute requests in the standard Command Prompt (cmd).
